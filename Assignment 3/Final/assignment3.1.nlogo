...
vacuums-own [beliefs desire intention performed-cleaning]
...
to go
  ...
  update-desires
  update-beliefs
  ; if all the dirt is gone, stop the main loop. This is when the agent has the belief that there is no dirt in the room anymore.
  if exit
    [ stop ]
...
to setup-patches
  ...
  ; calculate the total number of patches in the grid, add 1 for the "zero column" and "zero row"
  set num_of_tiles ((max-pxcor * 2) + 1) * ((max-pycor * 2) + 1)
  set total_dirty round (dirt_pct * num_of_tiles / 100)
  if num_of_tiles = total_dirty [ error "The dirty percentage is too high. Please lower the percentage!" ]
...
to update-desires
  ...
  ask vacuums [
    ifelse length beliefs >= 1
        ; the desire is clean all the  dirty patches in the environmnet. If this desire is true,
        ; there are some dirty patches left, if not then all the dirt is gone and the desire to clean the environment is false
        [ set desire true ]
        [
          set desire false
          set exit true
...
to update-intentions
  ...
  ask vacuums [
    ; after performing an action the intention is set to 0 (empty). If the intention is empty, set a new intention
    if intention = 0
       [ set intention item 0 beliefs ]
    ; if the agent is located on "the" dirty patch that it intended to navigate to,
    ; change the intention to "clean-dirt" (clean that patch)
...
to execute-actions
  ...
  ask vacuums [
    ...
    [
      ; intention is still to navigate to the intended patch
      face intention
      forward 1
    ]
    [ clean-dirt ]
...
to clean-dirt
  ...
  ask vacuums [
    if pcolor = grey and intention = "clean-dirt" [
      set pcolor black
      ; reset intention and indicate that agent just cleaned a patch
      set intention  0
